...
class QuestionsList extends Component {
  constructor() {
    ...
    this.verifyCharacter = this.verifyCharacter.bind(this);
  }
  ...
  verifyCharacter() {
    const { index } = this.state;
    const { results } = this.props;
    const hasSigleQuot = results[index].question.includes('&#039;');
    const hasDobleQuot = results[index].question.includes('&quot;');
    if (hasSigleQuot) {
      results[index].question = results[index].question.replace(/&#039;/g, '\'');
    }
    if (hasDobleQuot) {
      results[index].question = results[index].question.replace(/&quot;/g, '"');
    }
  ...
  render() {
    ...
    let newResults = [];
    if (results.length > 0) {
      newResults = this.verifyCharacter();
    }
